package com.sulake.core.utils
{
        ...
        public class LibraryLoader extends EventDispatcherWrapper implements IDisposable
    {
        ...
        protected static function addRequestCounterToUrlRequest(request:URLRequest, counter:int):void
        {
            ...
            var _local_4:Array = request.url.split("?");
            ...
            while (_local_3 < _local_10.length)
            {
                ...
                {
                    _local_6 = ((_local_7 + "=") + counter.toString());
            ...
            if (!_local_8)
            {
                _local_10.push(((_local_7 + "=") + counter));
            };
            ...
            request.url = _local_5;
        }

        protected static function parseNameFromUrl(url:String):String
        {
            var _local_2:int;
            _local_2 = url.indexOf("?", 0);

            if (_local_2 > -1)
            {
                url = url.slice(0, _local_2);
            };

            _local_2 = url.lastIndexOf(".");
            ...
            if (_local_2 > -1)
            {
                url = url.slice((_local_2 + 1), url.length);
            };
        ...
        {
            if (_SafeStr_870.indexOf(loader) == -1)
            {
                _SafeStr_870.push(loader);
            };
        ...
        protected function handleHttpStatus(status:int):Boolean
        {
            if (((status == 0) || (status >= 400)))
            {
                ...
                failure((((("HTTP Error " + status) + ' "') + _SafeStr_869) + '"'));
                removeEventListeners();
            };
