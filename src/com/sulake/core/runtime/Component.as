package com.sulake.core.runtime
{
        ...
        public class Component implements IUnknown, ICoreConfiguration
    {
        ...
        private var _dependencyDisposalActions:Vector.<Function> = new Vector.<Function>(0);

        public function Component(_arg_1:IContext, _arg_2:uint=0, assets:IAssetLibrary=null)
        {
            _flags = _arg_2;
            _iids = new InterfaceStructList();
            _events = new EventDispatcherWrapper();
            _context = _arg_1;
            _assets = ((assets != null) ? assets : new AssetLibrary("_internal_asset_library"));
        ...
        {
            var identifier:IID = identifier;
            var dependencySetter:Function = dependencySetter;
            var isRequired:Boolean = isRequired;
            var eventListeners:Array = eventListeners;

            if (isRequired)
            {
                _requiredDependencyCount++;
            };

            queueInterface(identifier, (function (identifier:Function, dependencySetter:Boolean, isRequired:Array, eventListeners:String):Function
            {
                var setter:Function = identifier;
                var required:Boolean = dependencySetter;
                var listeners:Array = isRequired;
                var componentName:String = eventListeners;
                return (function (identifier:IID, dependencySetter:IUnknown):void
                {
                    var identifier:IID = identifier;
                    var unknown:IUnknown = dependencySetter;
                    ...
                    _dependencyDisposalActions.push((function (identifier:IID, dependencySetter:IUnknown):Function
                    {
                        var iid:IID = identifier;
                        var component:IUnknown = dependencySetter;
        ...
        {
            return (_requiredDependencyCount == 0);
        }
        ...
        {
            var _local_3:InterfaceStruct = _iids.getStructByInterface(_arg_1);
        ...
        public function release(_arg_1:IID):uint
        {
            ...
            var _local_2:InterfaceStruct = _iids.getStructByInterface(_arg_1);
            ...
            if ((_flags & 0x04))
            {
                if (_local_3 == 0)
                {
                    if (_iids.getTotalReferenceCount() == 0)
                    {
        ...
        public function dispose():void
        {
            if (!_disposed)
            {
                for each (var _local_1:Function in _dependencyDisposalActions)
                {
                    (_local_1());
                };

                _dependencyDisposalActions = null;
        ...
        public function toXMLString(_arg_1:uint=0):String
        {
            ...
            var _local_6:uint = _iids.length;
            _local_8 = 0;

            while (_local_8 < _local_6)
            {
                _local_3 = _iids.getStructByIndex(_local_8);
