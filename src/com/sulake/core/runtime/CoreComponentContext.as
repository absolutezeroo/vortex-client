package com.sulake.core.runtime
{
        ...
        public final class CoreComponentContext extends ComponentContext implements ICore
    {
        ...
        public function hibernate(hibernationLevel:int, duration:int=1):void
        {
            ...
            {
                PurgeTrigger.stop();
                _hibernationLevel = hibernationLevel;
                _hibernationUpdateFrequency = (1000 / duration);
            };
        }
            ...
            while (_local_7 < maxPriority)
            {
                ...
                while (_local_6 != _local_5)
                {
                    ...
                    else
                    {
                        try
                        {
                            _local_3.update(msSinceLastUpdate);
                        }
        ...
        private function complexFrameUpdateHandler(msCurrentTime:uint, msSinceLastUpdate:uint):void
        {
            ...
            while (_local_11 < maxPriority)
            {
                ...
                if ((_local_5 - msCurrentTime) > _local_8)
                {
                ...
                if (!_local_10)
                {
                    ...
                    while (((!(_local_9 == _local_7)) && (_local_3)))
                    {
                        ...
                        else
                        {
                            try
                            {
                                _local_4.update(msSinceLastUpdate);
                            }
            ...
            while (_local_7 < maxPriority)
            {
                ...
                while (_local_6 != _local_5)
                {
                    ...
                    else
                    {
                        try
                        {
                            _SafeStr_848.update(_local_3, msSinceLastUpdate);
                        }
...
class UpdateDelegate implements IDisposable
{
    ...
    public function UpdateDelegate(receiver:IUpdateReceiver, context:IContext, priority:int)
    {
        if (((context) && (receiver)))
        {
            ...
            _context = context;
            _priority = priority;
            context.displayObjectContainer.stage.addEventListener(((_priority == 0) ? "exitFrame" : "enterFrame"), onFrameUpdate);
            _lastUpdateTimeMs = getTimer();
        };
    }
