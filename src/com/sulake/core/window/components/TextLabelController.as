package com.sulake.core.window.components
{
    ...
    public class TextLabelController extends WindowController implements ILabelWindow, ILocalizable
    {
        ...
        {
            return ((_margins) ? _margins.left : 0);
        }

        public function get drawOffsetY():int
        {
            return ((_margins) ? _margins.top : 0);
        }
        ...
        private function refresh(_arg_1:Boolean=false):void
        {
            ...
            _local_11 = ((_margins) ? (_margins.left + _margins.right) : 0);

            var _local_10:int = ((_margins) ? (_margins.top + _margins.bottom) : 0);
        ...
        {
            if (((_arg_1) && (!(_arg_1 == _margins))))
            {
                if (_margins)
                {
                    _margins.assign(_arg_1.left, _arg_1.top, _arg_1.right, _arg_1.bottom, setTextMargins);
                }

                else
                {
                    _margins = new TextMargins(_arg_1.left, _arg_1.top, _arg_1.right, _arg_1.bottom, setTextMargins);
                };
            };
        ...
        override public function get properties():Array
        {
            ...
            if (_margins != null)
            {
                _local_3.push(createProperty("margin_left", _margins.left));
                _local_3.push(createProperty("margin_top", _margins.top));
                _local_3.push(createProperty("margin_right", _margins.right));
                _local_3.push(createProperty("margin_bottom", _margins.bottom));
            }
            ...
            for each (var _local_2:PropertyStruct in _arg_1)
            {
                switch (_local_2.key)
                {
                    ...
                    case "margin_left":
                        ...
                        if (((!(_margins == null)) || (_local_2.valid)))
                        {
