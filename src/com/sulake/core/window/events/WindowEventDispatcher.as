package com.sulake.core.window.events
{
    ...
    public class WindowEventDispatcher implements IDisposable 
    {
        ...
        {
            var _local_6:Array = _eventListenerTable[type];
            var _local_4:EventListenerStruct = new EventListenerStruct(callback, false, priority);

            if (!_local_6)
            {
                _local_6 = [_local_4];
                _eventListenerTable[type] = _local_6;
            }
            ...
            else
            {
                ...
                {
                    if (_local_5.callback == callback)
                    {
                        return;
                    };

                    if (priority > _local_5.priority)
                    {
        ...
        public function removeEventListener(type:String, callback:Function):void
        {
            ...
            if (!_disposed)
            {
                _local_4 = _eventListenerTable[type];
        ...
        public function dispatchEvent(event:WindowEvent):Boolean
        {
            ...
            {
                _result = 0;
                _local_3 = _eventListenerTable[event.type];
        ...
        {
            return ((_disposed) ? false : (!(_eventListenerTable[type] == null)));
        }
        ...
        public function dispose():void
        {
            ...
            if (!_disposed)
            {
                for (var _local_3:Object in _eventListenerTable)
                {
