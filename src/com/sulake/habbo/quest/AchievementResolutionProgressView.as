package com.sulake.habbo.quest
{
    ...
    public class AchievementResolutionProgressView implements IDisposable 
    {
        ...
        private function setProgress(_arg_1:int, _arg_2:int):void
        {
            ...
            _window.findChildByName("achieved_mid").width = (_progressSteps * _local_3);
            _controller.questEngine.localization.registerParameter("resolution.progress.progress", "progress", _arg_1.toString());
            _controller.questEngine.localization.registerParameter("resolution.progress.progress", "total", _arg_2.toString());
        }
        ...
        {
            _window.findChildByName("achievement.name").caption = _controller.questEngine.localization.getBadgeName(_badgeCode);
            _window.findChildByName("achievement.desc").caption = _controller.questEngine.localization.getBadgeDesc(_badgeCode);
        }
        ...
        {
            _window = IFrameWindow(_controller.questEngine.getXmlWindow("AchievementResolutionProgress"));
            _window.findChildByTag("close").procedure = onWindowClose;
            _window.findChildByName("reset_button").procedure = onResetButton;
            _progressSteps = _window.findChildByName("achieved_mid").width;
        }
        ...
        private function onResetButton(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            if (_arg_1.type == "WME_CLICK")
            {
                _controller.resetResolution(_stuffId);
                close();
            };
        }
