package com.sulake.habbo.room.object.logic
{
    ...
    public class AvatarLogic extends MovingObjectLogic
    {
        ...
        {
            _blinkingStartTimeStamp = (getTimer() + getBlinkInterval());
        }
        ...
        override public function processUpdateMessage(_arg_1:RoomObjectUpdateMessage):void
        {
            ...
            if ((_arg_1 is RoomObjectAvatarChatUpdateMessage))
            {
                ...
                _talkingEndTimeStamp = (getTimer() + (_local_22.numberOfWords * 1000));
                return;
            };
            ...
            if ((_arg_1 is RoomObjectAvatarExpressionUpdateMessage))
            {
                ...
                _animationEndTimeStamp = AvatarAction.getExpressionTime(_local_14.getNumber("figure_expression"));

                if (_animationEndTimeStamp > -1)
                {
                    _animationEndTimeStamp = (_animationEndTimeStamp + getTimer());
                };
            ...
            if ((_arg_1 is RoomObjectAvatarPlayerValueUpdateMessage))
            {
                ...
                _numberValueEndTimeStamp = (getTimer() + 3000);
                return;
            };
            ...
            if ((_arg_1 is RoomObjectAvatarCarryObjectUpdateMessage))
            {
                ...
                _carryObjectStartTimeStamp = getTimer();

                if (_local_24.itemType < 999999999)
                {
                    _carryObjectEndTimeStamp = 0;
                    _allowUseCarryObject = (_local_24.itemType <= 999);
                }

                else
                {
                    _carryObjectEndTimeStamp = (_carryObjectStartTimeStamp + 1500);
            ...
            if ((_arg_1 is RoomObjectAvatarSignUpdateMessage))
            {
                ...
                _signEndTimeStamp = (getTimer() + 5000);
                return;
            };
        ...
        private function updateEffect(_arg_1:int, _arg_2:int, _arg_3:IRoomObjectModelController):void
        {
            if (_arg_1 == 28)
            {
                _effectChangeTimeStamp = (getTimer() + 500);
                _newEffect = 29;
            }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (_arg_2 == 0)
                            {
                                _effectChangeTimeStamp = 0;
                            }

                            else
                            {
                                _effectChangeTimeStamp = (getTimer() + _arg_2);
                                _newEffect = _arg_1;
                                return;
                            };
                        };
        ...
        private function updateActions(_arg_1:int, _arg_2:IRoomObjectModelController):void
        {
            if (_talkingEndTimeStamp > 0)
            {
                if (_arg_1 > _talkingEndTimeStamp)
                {
                    ...
                    _talkingPauseStartTimeStamp = 0;
                    _talkingPauseEndTimeStamp = 0;
                }

                else
                {
                    if (((_talkingPauseEndTimeStamp == 0) && (_talkingPauseStartTimeStamp == 0)))
                    {
                        _talkingPauseStartTimeStamp = (_arg_1 + getTalkingPauseInterval());
                        _talkingPauseEndTimeStamp = (_talkingPauseStartTimeStamp + getTalkingPauseLength());
                    }

                    else
                    {
                        if (((_talkingPauseStartTimeStamp > 0) && (_arg_1 > _talkingPauseStartTimeStamp)))
                        {
                            _arg_2.setNumber("figure_talk", 0);
                            _talkingPauseStartTimeStamp = 0;
                        }
                        ...
                        {
                            if (((_talkingPauseEndTimeStamp > 0) && (_arg_1 > _talkingPauseEndTimeStamp)))
                            {
            ...
            if (((_animationEndTimeStamp > 0) && (_arg_1 > _animationEndTimeStamp)))
            {
                _arg_2.setNumber("figure_expression", 0);
                _animationEndTimeStamp = 0;
            };
            ...
            if (((_signEndTimeStamp > 0) && (_arg_1 > _signEndTimeStamp)))
            {
                _arg_2.setNumber("figure_sign", -1);
                _signEndTimeStamp = 0;
            };

            if (_carryObjectEndTimeStamp > 0)
            {
                if (_arg_1 > _carryObjectEndTimeStamp)
                {
                    _arg_2.setNumber("figure_carry_object", 0);
                    _arg_2.setNumber("figure_use_object", 0);
                    _carryObjectStartTimeStamp = (_carryObjectEndTimeStamp = 0);
                    _allowUseCarryObject = false;
                };
            };
            ...
            {
                if ((_arg_1 - _carryObjectStartTimeStamp) > 5000)
                {
                    if (((_arg_1 - _carryObjectStartTimeStamp) % 10000) < 1000)
                    {
            ...
            if (_arg_1 > _blinkingStartTimeStamp)
            {
                _arg_2.setNumber("figure_blink", 1);
                _blinkingStartTimeStamp = (_arg_1 + getBlinkInterval());
                _blinkingEndTimeStamp = (_arg_1 + getBlinkLength());
            };

            if (((_blinkingEndTimeStamp > 0) && (_arg_1 > _blinkingEndTimeStamp)))
            {
                _arg_2.setNumber("figure_blink", 0);
                _blinkingEndTimeStamp = 0;
            };

            if (((_effectChangeTimeStamp > 0) && (_arg_1 > _effectChangeTimeStamp)))
            {
                _arg_2.setNumber("figure_effect", _newEffect);
                _effectChangeTimeStamp = 0;
            };

            if (((_numberValueEndTimeStamp > 0) && (_arg_1 > _numberValueEndTimeStamp)))
            {
                _arg_2.setNumber("figure_number_value", 0);
                _numberValueEndTimeStamp = 0;
            };
        }
