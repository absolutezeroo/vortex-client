package com.sulake.habbo.room.object.visualization.furniture
{
    ...
    public class FurnitureFireworksVisualization extends AnimatedFurnitureVisualization 
    {
        ...
        override protected function updateObject(_arg_1:Number, _arg_2:Number):Boolean
        {
            ...
            if (super.updateObject(_arg_1, _arg_2))
            {
                ...
                if (!_particleSystems)
                {
                    ...
                    if (_particleSystems)
                    {
                        _currentParticleSystem = _particleSystems[_arg_1];
                        _local_3 = true;
                    }
                ...
                {
                    if (((!(_arg_1 == _SafeStr_3272)) || (!(_particleSystems[_arg_1] == _currentParticleSystem))))
                    {
                        _local_4 = _particleSystems[_arg_1];
                        _local_4.copyStateFrom(_currentParticleSystem);

                        if (_currentParticleSystem)
                        {
                            _currentParticleSystem.reset();
                        };

                        _currentParticleSystem = _local_4;
                        _local_3 = true;
                    };
                };
        ...
        override protected function updateSprites(_arg_1:int, _arg_2:Boolean, _arg_3:int):void
        {
            ...
            if (_currentParticleSystem)
            {
                _currentParticleSystem.updateSprites();
            };
        }
        ...
        override protected function updateAnimation(_arg_1:Number):int
        {
            if (_currentParticleSystem)
            {
                _currentParticleSystem.updateAnimation();
            };
        ...
        override protected function setAnimation(_arg_1:int):void
        {
            if (_currentParticleSystem)
            {
                _currentParticleSystem.setAnimation(_arg_1);
            };
        ...
        {
            if (((_currentParticleSystem) && (_currentParticleSystem.controlsSprite(_arg_3))))
            {
                return (_currentParticleSystem.getSpriteYOffset(_arg_1, _arg_2, _arg_3));
            };
