package com.sulake.habbo.room.preview
{
    ...
    public class RoomPreviewer 
    {
        ...
        public function reset(_arg_1:Boolean):void
        {
            ...
            _currentPreviewObjectCategory = -2;
        }
        ...
        public function addFurnitureIntoRoom(_arg_1:int, _arg_2:IVector3d, _arg_3:IStuffData=null, _arg_4:String=null):int
        {
            ...
            {
                if (((_currentPreviewObjectCategory == 10) && (_currentPreviewObjectType == _arg_1)))
                {
                    return (1);
                };

                reset(false);
                _currentPreviewObjectType = _arg_1;
                _currentPreviewObjectCategory = 10;
                ...
                if (_roomEngine.addObjectFurniture(_previewRoomId, 1, _arg_1, new Vector3d(2, 2, 0), _arg_2, 0, _arg_3, NaN, -1, 0, 0, "", true, false))
                {
                    ...
                    _local_5 = 1;
                    _local_6 = _roomEngine.getRoomObject(_previewRoomId, 1, _currentPreviewObjectCategory);
        ...
        public function addWallItemIntoRoom(_arg_1:int, _arg_2:IVector3d, _arg_3:String):int
        {
            ...
            if (isRoomEngineReady)
            {
                if ((((_currentPreviewObjectCategory == 20) && (_currentPreviewObjectType == _arg_1)) && (_currentPreviewObjectData == _arg_3)))
                {
                    return (1);
                };
                ...
                reset(false);
                _currentPreviewObjectType = _arg_1;
                _currentPreviewObjectCategory = 20;
                _currentPreviewObjectData = _arg_3;
        ...
        public function addAvatarIntoRoom(_arg_1:String, _arg_2:int):int
        {
            if (isRoomEngineReady)
            {
                ...
                _currentPreviewObjectCategory = 100;
                _currentPreviewObjectData = _arg_1;
        ...
        public function changeRoomObjectState():void
        {
            if (isRoomEngineReady)
            {
                ...
                if (_currentPreviewObjectCategory != 100)
                {
                    _roomEngine.changeObjectState(_previewRoomId, 1, _currentPreviewObjectCategory);
                };
            };
        }
        ...
        public function getRoomCanvas(_arg_1:int, _arg_2:int):DisplayObject
        {
            ...
            if (_roomEngine)
            {
                _local_3 = _roomEngine.createRoomCanvas(_previewRoomId, 1, _arg_1, _arg_2, _currentPreviewScale);
                ...
                _currentPreviewCanvasHeight = _arg_2;
                return (_local_3);
            };
        ...
        private function updatePreviewObjectBoundingRectangle(_arg_1:Point):void
        {
            ...
            var _local_3:Rectangle = _roomEngine.getRoomObjectBoundingRectangle(_previewRoomId, 1, _currentPreviewObjectCategory, 1);

            if (((!(_local_3 == null)) && (!(_arg_1 == null))))
            {
                _local_3.offset(-(_currentPreviewCanvasWidth >> 1), -(_currentPreviewCanvasHeight >> 1));
                ...
                else
                {
                    ...
                    if ((((((_local_2.width - _currentPreviewRectangle.width) > ((_currentPreviewCanvasWidth - _currentPreviewRectangle.width) >> 1)) || ((_local_2.height - _currentPreviewRectangle.height) > ((_currentPreviewCanvasHeight - _currentPreviewRectangle.height) >> 1))) || (_currentPreviewRectangle.width < 1)) || (_currentPreviewRectangle.height < 1)))
                    {
        ...
        private function validatePreviewSize(_arg_1:Point):Point
        {
            ...
            if (isRoomEngineReady)
            {
                ...
                if (((_currentPreviewRectangle.width > (_currentPreviewCanvasWidth * (1 + 0.25))) || (_currentPreviewRectangle.height > (_currentPreviewCanvasHeight * (1 + 0.25)))))
                {
                ...
                {
                    if ((((_currentPreviewRectangle.width << 1) < ((_currentPreviewCanvasWidth * (1 + 0.25)) - 5)) && ((_currentPreviewRectangle.height << 1) < ((_currentPreviewCanvasHeight * (1 + 0.25)) - 5))))
                    {
                        if ((_roomEngine as Component).getBoolean("zoom.enabled"))
                        {
                            if (((!(_roomEngine.getRoomCanvasScale(_previewRoomId, 1) == 1)) && (!(_currentPreviewNeedsZoomOut))))
                            {
                                _roomEngine.setRoomCanvasScale(_previewRoomId, 1, 1, null, null, false, false, true);
                                _currentPreviewScale = 64;
                                _arg_1.x = (_arg_1.x << 1);
                                _arg_1.y = (_arg_1.y << 1);
                            };
                        }
                        ...
                        {
                            if (((!(_local_2.isZoomedIn())) && (!(_currentPreviewNeedsZoomOut))))
                            {
        ...
        private function getCanvasOffset(_arg_1:Point):Point
        {
            ...
            var _local_3:Number = ((_currentPreviewCanvasHeight - _currentPreviewRectangle.height) >> 1);
            ...
            if (isRoomEngineReady)
            {
                ...
                if (_local_3 != null)
                {
                    ...
                    if (_currentPreviewRectangle != null)
                    {
                        ...
                        if (_currentPreviewScale != _local_4)
                        {
        ...
        private function onRoomObjectAdded(_arg_1:RoomEngineObjectEvent):void
        {
            ...
            if ((((_arg_1.roomId == _previewRoomId) && (_arg_1.objectId == 1)) && (_arg_1.category == _currentPreviewObjectCategory)))
            {
                _currentPreviewRectangle = null;
