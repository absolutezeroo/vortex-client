package com.sulake.habbo.window.widgets
{
    ...
    public class RoomPreviewerWidget implements IRoomPreviewerWidget
    {
        ...
        public function RoomPreviewerWidget(_arg_1:IWidgetWindow, _arg_2:HabboWindowManagerComponent)
        {
            ...
            _root = (_windowManager.buildFromXML((_windowManager.assets.getAssetByName("room_previewer_xml").content as XML)) as IWindowContainer);
            _roomCanvas = (_root.findChildByName("room_canvas") as IDisplayObjectWrapper);
            _roomPreviewer = new RoomPreviewer(_arg_2.roomEngine, ROOM_ID_COUNTER++);
            _roomPreviewer.createRoomForPreviews();
            _root.addEventListener("WME_CLICK", onClickRoomView);
            _root.addEventListener("WE_RESIZE", onResizeCanvas);
            _widgetWindow.rootWindow = _root;
            _root.width = _widgetWindow.width;
            _root.height = _widgetWindow.height;
            _roomPreviewer.modifyRoomCanvas(_root.width, _root.height);
        }
            ...
            switch (_arg_1.type)
            {
                case "REE_INITIALIZED":
                    ...
                    if (((_roomPreviewer) && (_arg_1.roomId == _roomPreviewer.previewRoomId)))
                    {
                        ...
                        _local_2 = _roomPreviewer.getRoomCanvas(_roomCanvas.width, _roomCanvas.height);

                        if (_local_2 != null)
                        {
                            _roomCanvas.setDisplayObject(_local_2);
                        };
                    };
        ...
        private function refresh():void
        {
            ...
            if (((_roomPreviewer) && (_roomPreviewer.isRoomEngineReady)))
            {
                ...
                _local_1 = _roomCanvas.getDisplayObject();
