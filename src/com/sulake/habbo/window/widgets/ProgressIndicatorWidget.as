package com.sulake.habbo.window.widgets
{
    ...
    public class ProgressIndicatorWidget implements IProgressIndicatorWidget
    {
        ...
        public function ProgressIndicatorWidget(_arg_1:IWidgetWindow, _arg_2:HabboWindowManagerComponent)
        {
            ...
            _root = (_windowManager.buildFromXML((_windowManager.assets.getAssetByName("progress_indicator_xml").content as XML)) as IItemListWindow);
        ...
        public function set size(_arg_1:uint):void
        {
            ...
            if (_arg_1 != size)
            {
                while (_arg_1 < size)
                {
                    _root.removeListItemAt((size - 1));
                };

                while (_arg_1 > size)
                {
                    _root.addListItem(_root.getListItemAt(0).clone());
                };
        ...
        private function refresh():void
        {
            ...
            while (_local_3 < size)
            {
                _local_4 = (_root.getListItemAt(_local_3) as IStaticBitmapWrapperWindow);
                switch (_mode)
                {
                    case "position":
