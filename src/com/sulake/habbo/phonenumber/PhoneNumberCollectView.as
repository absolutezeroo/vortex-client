package com.sulake.habbo.phonenumber
{
    ...
    public class PhoneNumberCollectView 
    {
        ...
        private function createWindow(_arg_1:Array):void
        {
            ...
            _window = IFrameWindow(_component.windowManager.buildFromXML(XML(_component.assets.getAssetByName("phonenumber_collect_xml").content)));
            _window.center();
            ...
            var _local_6:String = ((_component.localizationManager.getLocalization("phone.number.collect.countries")) || ("{}"));
            ...
            for each (var _local_2:Object in _locales)
            {
        ...
        {
            var _local_3:IWindowContainer = IWindowContainer(_component.windowManager.buildFromXML(XML(_component.assets.getAssetByName("phonenumber_country_menu_item_xml").content)));
        ...
        private function onInput(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            if (_arg_1.type == "WME_DOWN")
            {
                switch (_arg_2.name)
                {
                    ...
                    case "skip_link":
                        _component.setCollectViewMinimized(true);
                        break;
                    case "never_link":
                        _component.windowManager.confirm("${phone.number.never.again.confirm.title}", "${phone.number.never.again.confirm.text}", 0, onNeverAgainConfirmClose);
                        break;
                    case "ok_button":
                        _component.sendTryPhoneNumber(selectedCountryCode, _window.findChildByName("phone_number_input").caption);
