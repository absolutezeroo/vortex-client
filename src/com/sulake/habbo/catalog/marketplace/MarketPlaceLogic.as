package com.sulake.habbo.catalog.marketplace
{
    ...
    public class MarketPlaceLogic implements IMarketPlace
    {
        ...
        private function showConfirmation(_arg_1:int, _arg_2:MarketPlaceOfferData):void
        {
            ...
            {
                _confirmationDialog = new MarketplaceConfirmationDialog(this, _catalog, _roomEngine);
            };

            _confirmationDialog.showConfirmation(_arg_1, _arg_2);
        }
        ...
        {
            requestOffers(_minPrice, _maxPrice, _searchString, _sortOrder);
        }
        ...
        public function requestItemStats(_arg_1:int, _arg_2:int):void
        {
            if (_catalog)
            {
                _statsRequestTypeId = _arg_2;
                _statsRequestCategoryId = _arg_1;
        ...
        public function onOffers(_arg_1:IMessageEvent):void
        {
            ...
            _totalItemsFound = _local_2.totalItemsFound;
        ...
        public function onBuyResult(event:IMessageEvent):void
        {
            ...
            else
            {
                if (parser.result == 2)
                {
                    ...
                    {
                        _windowManager.alert("${catalog.marketplace.not_available_title}", "${catalog.marketplace.not_available_header}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                        {
                            event.dispose();
                        });
                    };
                ...
                else
                {
                    ...
                    else
                    {
                        if (parser.result == 4)
                        {
                            if (_windowManager != null)
                            {
                                _windowManager.alert("${catalog.alert.notenough.title}", "${catalog.alert.notenough.credits.description}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                                {
                                    event.dispose();
                                });
                            };
        ...
        public function onCancelResult(event:IMessageEvent):void
        {
            ...
            else
            {
                if (_windowManager != null)
                {
                    _windowManager.alert("${catalog.marketplace.operation_failed.topic}", "${catalog.marketplace.cancel_failed}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                    {
                        event.dispose();
                    });
                };
        ...
        {
            if (((!(_arg_1.furniCategoryId == _statsRequestCategoryId)) || (!(_arg_1.furniTypeId == _statsRequestTypeId))))
            {
                return;
            };
