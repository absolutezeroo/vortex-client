package com.sulake.habbo.catalog.navigation
{
    ...
    public class CatalogNodeRenderable extends CatalogNode 
    {
        ...
        public function removeFromList(_arg_1:IItemListWindow):void
        {
            ...
            if (isBranch)
            {
                _arg_1.removeListItem(_childList);
            };
        }
        ...
        private function refreshChildren():void
        {
            ...
            for each (var _local_2:ICatalogNode in children)
            {
                ...
                if (_local_1 != null)
                {
                    if (_local_1.visible)
                    {
                        _local_1.addToList(_childList);
                        _local_1.setInactiveLook();
                    }

                    else
                    {
                        _local_1.removeFromList(_childList);
                    };
                };
            };

            _childList.arrangeListItems();
        }
        ...
        private function showChildren():void
        {
            ...
            for each (var _local_3:ICatalogNode in children)
            {
                if (_local_3.visible)
                {
                    (_local_3 as CatalogNodeRenderable).addToList(_childList);
                };
            };

            if (_childList != null)
            {
                _childList.visible = true;
                _local_1 = 0;
                _local_2 = 0;

                while (_local_2 < _childList.numListItems)
                {
                    if (_childList.getListItemAt(_local_2).visible)
                    {
                        _local_1++;
                    };
        ...
        private function removeChildren():void
        {
            for each (var _local_1:ICatalogNode in children)
            {
                if (_local_1.visible)
                {
                    (_local_1 as CatalogNodeRenderable).removeFromList(_childList);
                };
            };
        ...
        {
            _childList = (navigator.listTemplate.clone() as IItemListWindow);
            removeChildren();
        }
        ...
        public function updateChildListHeight():void
        {
            ...
            if (_childList != null)
            {
                ...
                while (_local_2 < _childList.numListItems)
                {
                    if (_childList.getListItemAt(_local_2).visible)
                    {
                        _childList.height = (_childList.height + _childList.getListItemAt(_local_2).height);
                    };
