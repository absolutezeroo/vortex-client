package com.sulake.habbo.catalog.viewer.widgets
{
    ...
    public class MarketPlaceCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener, IMarketPlaceVisualization 
    {
        ...
        override public function dispose():void
        {
            ...
            if (_listItemBase)
            {
                _listItemBase.dispose();
                _listItemBase = null;
            };

            if (_populationTimer)
            {
                _populationTimer.removeEventListener("timer", onPopulationTimer);
        ...
        override public function init():Boolean
        {
            ...
            _listItemBase = (_local_1.removeListItem(_local_1.getListItemByName("offer_item")) as IWindowContainer);
            return (true);
        }
        ...
        private function selectSearchCategory(_arg_1:String):void
        {
            ...
            if (_local_6 != null)
            {
                _local_6.populate(getSortKeys(_sortTypes));
        ...
        private function updateList():void
        {
            ...
            if (!_populationTimer)
            {
                _populationTimer = new Timer(25);
                _populationTimer.addEventListener("timer", onPopulationTimer);
            };
        ...
        private function populateList():Boolean
        {
            ...
            while (_local_1 < 5)
            {
                if (_populationIndex >= _offers.length)
                {
                    return (true);
                };

                addListItem(_offers.getWithIndex(_populationIndex));
                _populationIndex++;
                _local_1++;
            };
        ...
        private function addListItem(_arg_1:MarketPlaceOfferData):void
        {
            ...
            if (((((!(_arg_1)) || (!(_itemList))) || (!(_listItemBase))) || (!(marketPlace.localization))))
            {
                return;
            };

            var _local_14:IWindowContainer = (_listItemBase.clone() as IWindowContainer);
        ...
        private function doSearch():void
        {
            ...
            if ((((_local_5) && (_local_5.selection >= 0)) && (_local_5.selection < _sortTypes.length)))
            {
                _local_3 = _sortTypes[_local_5.selection];
            };
            ...
            if (_arg_1.type == "WME_CLICK")
            {
                switch (_arg_2.name)
                {
                    case "back_button":
                        ...
                        return;
                    case "buy_button":
                        marketPlace.buyOffer(_selectedOffer.offerId);
                        return;
                };
