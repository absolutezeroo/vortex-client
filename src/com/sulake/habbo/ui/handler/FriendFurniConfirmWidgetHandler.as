package com.sulake.habbo.ui.handler
{
    ...
    public class FriendFurniConfirmWidgetHandler implements IRoomWidgetHandler 
    {
        ...
        public function dispose():void
        {
            if (!_disposed)
            {
                if (((_connection) && (_friendFurniConfirmListener)))
                {
                    _connection.removeMessageEvent(_friendFurniConfirmListener);
                    _connection.removeMessageEvent(_friendFurniOtherLockConfirmListener);
                    _connection.removeMessageEvent(_friendFurniCancelLockListener);
                    _connection = null;
                };
        ...
        public function set connection(_arg_1:IConnection):void
        {
            ...
            if (!_friendFurniConfirmListener)
            {
                _friendFurniConfirmListener = new FriendFurniStartConfirmationMessageEvent(onStartConfirmation);
                _friendFurniOtherLockConfirmListener = new FriendFurniOtherLockConfirmedMessageEvent(onOtherLockConfirmed);
                _friendFurniCancelLockListener = new FriendFurniCancelLockMessageEvent(onCancelLock);
                _connection.addMessageEvent(_friendFurniConfirmListener);
                _connection.addMessageEvent(_friendFurniOtherLockConfirmListener);
                _connection.addMessageEvent(_friendFurniCancelLockListener);
            };
        }
