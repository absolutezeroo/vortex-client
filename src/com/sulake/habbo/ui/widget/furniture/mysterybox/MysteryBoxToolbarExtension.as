package com.sulake.habbo.ui.widget.furniture.mysterybox
{
    ...
    public class MysteryBoxToolbarExtension implements IDisposable 
    {
        ...
        {
            var _local_1:XML = (_handler.widget.assets.getAssetByName("mystery_box_toolbar_extension").content as XML);
            _window = (_handler.container.windowManager.buildFromXML(_local_1) as IWindowContainer);
            _window.findChildByName("faq_link").visible = (!(_handler.container.config.getProperty("mysterybox.faq.url") == ""));
            _window.procedure = windowProcedure;
            _handler.container.toolbar.extensionView.attachExtension("mystery_box", _window);

            var _local_2:ISessionDataManager = _handler.container.sessionDataManager;
        ...
        private function windowProcedure(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            ...
            switch (_arg_1.target.name)
            {
                ...
                case "maximize_region":
                    ...
                    return;
                case "faq_link":
                    HabboWebTools.openWebPage(_handler.container.config.getProperty("mysterybox.faq.url"), "habboMain");
                    return;
            };
        }
        ...
        {
            return ((!(_handler == null)) && (_handler.container.config.getBoolean("mystery_box_toolbar_extension_minimised")));
        }

        private function setMinimised(_arg_1:Boolean):void
        {
            if (((!(_handler == null)) && (!(_window == null))))
            {
                if (_arg_1)
                {
                ...
                _handler.container.config.setProperty("mystery_box_toolbar_extension_minimised", _arg_1.toString());
            };
        }
        ...
        public function dispose():void
        {
            ...
            if (_handler != null)
            {
                _handler.container.toolbar.extensionView.detachExtension("mystery_box");
                _handler.container.sessionDataManager.events.removeEventListener("mbke_update", onKeysUpdated);
                _handler = null;
            };
