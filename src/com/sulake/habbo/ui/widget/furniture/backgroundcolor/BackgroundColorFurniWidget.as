package com.sulake.habbo.ui.widget.furniture.backgroundcolor
{
    ...
    public class BackgroundColorFurniWidget extends RoomWidgetBase 
    {
        ...
        {
            _furniId = _arg_1;
            _hue = Math.max(_arg_2, 0);
            _saturation = Math.max(_arg_3, 0);
            _lightness = Math.max(_arg_4, 0);
            createWindow();
        }
        ...
        private function createWindow():void
        {
            if (!_window)
            {
                ...
                _window.center();
                _sliders.push(new BackgroundColorWidgetSlider(this, "hue", IWindowContainer(_window.findChildByName("hue_container")), _hue));
                ...
                _sliders.push(new BackgroundColorWidgetSlider(this, "lightness", IWindowContainer(_window.findChildByName("lightness_container")), _lightness));
            };
        }
        ...
        private function renderColorPreview():void
        {
            ...
            var _local_1:uint = ColorConverter.hslToRGB(((((_hue & 0xFF) << 16) + ((_saturation & 0xFF) << 8)) + (_lightness & 0xFF)));
        ...
        private function windowProcedure(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            if (((!(_arg_2 == null)) && (_arg_1.type == "WME_CLICK")))
            {
                ...
                {
                    case "apply_button":
                        handler.container.connection.send(new SetRoomBackgroundColorDataComposer(_furniId, _hue, _saturation, _lightness));
                        return;
                    case "on_off_button":
