package com.sulake.habbo.ui.widget.memenu
{
    ...
    public class MeMenuSettingsMenuView implements IMeMenuView 
    {
        ...
        {
            var _local_2:XmlAsset = (_widget.assets.getAssetByName("memenu_settings_menu") as XmlAsset);

            if (_local_2)
            {
                _window = (_widget.windowManager.buildFromXML((_local_2.content as XML)) as IWindowContainer);
            };
            ...
            if (((!(ExternalInterface.available)) || (!(_widget.config.getProperty("has.identity") == "1"))))
            {
        ...
        private function eventHandler(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            ...
            switch (_arg_2.name)
            {
                case "character_settings":
                    ...
                    return;
                case "sound_settings":
                    _widget.changeView("me_menu_sound_settings");
                    return;
                case "chat_settings":
                    _widget.changeView("me_menu_chat_settings");
                    return;
                case "back":
