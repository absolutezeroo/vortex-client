package com.sulake.habbo.help.guidehelp
{
    ...
    public class GuideSessionController implements IDisposable, IIlluminaInputHandler
    {
        ...
        public function GuideSessionController(_arg_1:GuideHelpManager)
        {
            ...
            _chatMsgReminder = IWindowContainer(_habboHelp.getXmlWindow("chat_msg_reminder"));
            _handleHelperRequests = _habboHelp.getBoolean("guidetool.handle.help_requests");
            _handleGuardianRequests = _habboHelp.getBoolean("guidetool.handle.chat_reviews");
            _handleGuideRequests = _habboHelp.getBoolean("guidetool.handle.tour_requests");
            _habboHelp.context.displayObjectContainer.stage.addEventListener("mouseMove", onStageMouseMove);
            _waitingAnimationData = new Vector.<AnimationData>();
            _waitingAnimationTimer = new Timer(500);
            _waitingAnimationTimer.addEventListener("timer", onWaitingAnimationTimer);
            _waitingAnimationTimer.start();
            _idleCheckLastActivity = getTimer();
            _idleCheckTimer = new Timer(5000);
            _idleCheckTimer.addEventListener("timer", onIdleCheckTimer);
        ...
        public function dispose():void
        {
            ...
            if (_idleCheckTimer != null)
            {
                ...
                _idleCheckTimer.removeEventListener("timer", onIdleCheckTimer);
                _idleCheckTimer = null;
            };

            if (_waitingAnimationTimer != null)
            {
                _waitingAnimationTimer.stop();
                _waitingAnimationTimer.removeEventListener("timer", onWaitingAnimationTimer);
                _waitingAnimationTimer = null;
            };

            _waitingAnimationData = null;
        ...
        {
            _habboHelp.sendMessage(new GuideSessionOnDutyUpdateMessageComposer(_onDuty, _handleGuideRequests, _handleHelperRequests, _handleGuardianRequests));
        }
        ...
        private function setStateGuideTool():void
        {
            ...
            setCheckBoxValue("handle_guardian_tickets", _handleGuardianRequests);
            setCheckBoxValue("handle_helper_tickets", _handleHelperRequests);
            setCheckBoxValue("handle_guide_tickets", _handleGuideRequests);
                ...
                case "guide_tool_duty":
                    switch (_arg_1.type)
                    {
                        case "WE_SELECTED":
                            ...
                            _handleHelperRequests = (_window.findChildByName("handle_helper_tickets") as _SafeStr_108).isSelected;
                            _handleGuardianRequests = ((!(_window.findChildByName("handle_guardian_tickets") == null)) && ((_window.findChildByName("handle_guardian_tickets") as _SafeStr_108).isSelected));
                            _handleGuideRequests = (_window.findChildByName("handle_guide_tickets") as _SafeStr_108).isSelected;

                            if ((((!(_handleHelperRequests)) && (!(_handleGuardianRequests))) && (!(_handleGuideRequests))))
                            {
                            ...
                            _habboHelp.sendMessage(new GuideSessionOnDutyUpdateMessageComposer(true, _handleGuideRequests, _handleHelperRequests, _handleGuardianRequests));
        ...
        {
            if (((_reportWindow) || (_window == null)))
            {
                return;
            };

            var _local_1:IDesktopWindow = _habboHelp.windowManager.getDesktop(0);
            _reportWindow = IWindowContainer(_habboHelp.getXmlWindow("report_window"));
            _reportWindow.procedure = onReportWindowEvent;
            _reportWindow.x = Math.max(0, Math.min((_local_1.width - _reportWindow.width), ((_window.x + _window.width) + 10)));
            _reportWindow.y = Math.max(0, _window.y);
        }
        ...
        private function onReportWindowEvent(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            ...
            if (((((disposed) || (!(_reportWindow))) || (_reportWindow.disposed)) || (!(_arg_1.type == "WME_CLICK"))))
            {
                return;
            };
            ...
            switch (_arg_2.name)
            {
                case "header_button_close":
                case "cancel_link":
                    _habboHelp.trackGoogle("guideHelp", (_reportWindow.name + "_clickClose"));
                    closeReportWindow();
                    return;
                case "urgent_help_link":
                    return;
                case "submit_button":
                    _local_3 = IIlluminaInputWidget(IWidgetWindow(_reportWindow.findChildByName("input_widget")).widget).message;

                    if (_local_3.length == 0)
                    {
                        _reportWindow.findChildByName("report_error").visible = true;
                        IItemListWindow(_reportWindow.findChildByName("list")).arrangeListItems();
                    }

                    else
                    {
                        _habboHelp.sendMessage(new GuideSessionReportMessageComposer(_local_3));
                        _habboHelp.trackGoogle("guideHelp", (_reportWindow.name + "_clickReport"));
                        closeReportWindow();
                        closeWindow();
                    };
        ...
        private function addSystemMessage(_arg_1:int, _arg_2:String):void
        {
            ...
            switch (_arg_1)
            {
                case 1:
                    ...
                    return;
                case 2:
                    _local_3 = (_chatMsgReminder.clone() as IWindowContainer);
                    _local_3.findChildByName("content").caption = _arg_2;
                    addItemAndUpdateChatList(_local_3);
                    return;
                default:
        ...
        private function resetTypingTimer():void
        {
            ...
            _typingTimer = new Timer(3000);
            _typingTimer.addEventListener("timer", onTypingTimer);
            _typingTimer.start();
        ...
        private function onWaitingAnimationTimer(_arg_1:TimerEvent):void
        {
            ...
            _waitingAnimationData = _waitingAnimationData.filter(function (_arg_1:AnimationData, _arg_2:int, _arg_3:Vector.<AnimationData>):Boolean
            {
                return ((!(_arg_1.window == null)) && (!(_arg_1.window.disposed)));
            });

            for each (var data:AnimationData in _waitingAnimationData)
            {
        ...
        private function startWaitingAnimation(_arg_1:IStaticBitmapWrapperWindow, _arg_2:String, _arg_3:int):void
        {
            ...
            _waitingAnimationData.push(_local_4);
        }

        private function setAnimationFrame(_arg_1:AnimationData):void
        {
            var _local_2:int = (_waitingAnimationTimer.currentCount % _arg_1.frameCount);
        ...
        {
            if (((_onDuty) && ((getTimer() - _idleCheckLastActivity) > (_habboHelp.getInteger("guidetool.idle.timeout", 300) * 1000))))
            {
                _habboHelp.sendMessage(new GuideSessionOnDutyUpdateMessageComposer(false, _handleGuideRequests, _handleHelperRequests, _handleGuardianRequests));
            };
        }
