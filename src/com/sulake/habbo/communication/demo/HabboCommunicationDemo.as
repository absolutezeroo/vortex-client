package com.sulake.habbo.communication.demo
{
        ...
        public class HabboCommunicationDemo extends Component
    {
        ...
        private function prepareProperties():void
        {
            ...
            Logger.log("[SSO] Using SSO ticket: " + _SafeStr_595);
        }
        ...
        public function sendConnectionParameters(_arg_1:IConnection):void
        {
            ...
            var _machineId:String = CommunicationUtils.readSOLString("machineid");

            if (((_machineId == null) || (_machineId.length == 0)))
            {
                _machineId = CommunicationUtils.generateFingerprint();
                CommunicationUtils.writeSOLProperty("machineid", _machineId);
            }

            var _fingerprint:String = CommunicationUtils.generateFingerprint();

            if (((_fingerprint == null) || (_fingerprint.length == 0)))
            {
                _fingerprint = _machineId;
            }

            var _flashVersionParts:Array = Capabilities.version.split(" ");
            var _flashClientVersion :String = _flashVersionParts.join("/");

            if (((_flashClientVersion == null) || (_flashClientVersion.length == 0)))
            {
                _flashClientVersion = Capabilities.version;
            }

            _arg_1.send(new UniqueIDMessageComposer(_machineId, _fingerprint, _flashClientVersion ));
