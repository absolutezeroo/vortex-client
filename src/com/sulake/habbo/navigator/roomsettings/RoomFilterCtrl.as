package com.sulake.habbo.navigator.roomsettings
{
    ...
    public class RoomFilterCtrl implements IDisposable 
    {
        ...
        private function prepareWindow():void
        {
            ...
            _badWordField = (_window.findChildByName("roomfilter_addword_txt") as ITextFieldWindow);
            _badWordList = IItemListWindow(_window.findChildByName("badwords_itemlist"));
            refreshBadWords();
            _window.center();
        }
        ...
        public function onRoomFilterSettings(_arg_1:Array):void
        {
            ...
            {
                if (_badWords.indexOf(_arg_1[_local_2]) == -1)
                {
                    _badWords.push(_arg_1[_local_2]);
                };
        ...
        private function refreshBadWords():void
        {
            ...
            _badWordList.autoArrangeItems = false;
            _local_2 = 0;

            while (true)
            {
                _local_1 = IWindowContainer(_badWordList.getListItemAt(_local_2));

                if (_local_1 == null)
                {
                    if (_badWords[_local_2] == null) break;
                    _local_1 = getListEntry(_local_2);
                    _badWordList.addListItem(_local_1);
                };

                if (_badWords[_local_2] != null)
                {
                    _local_1.color = this.getBgColor(_local_2, false);
                    refreshEntryDetails(_local_1, _badWords[_local_2]);
            ...
            _badWordList.autoArrangeItems = true;
            _badWordList.invalidate();
        }
        ...
        private function onRemoveWordClick(_arg_1:WindowMouseEvent):void
        {
            ...
            var _local_2:IWindowContainer = IWindowContainer(_badWordList.getListItemAt(_selectedRow));
            ...
            if (_badWords.indexOf(_local_3) >= 0)
            {
                _badWords.splice(_badWords.indexOf(_local_3), 1);
            };
        ...
        private function refreshColorsAfterClick(_arg_1:IItemListWindow):void
        {
            ...
            while (_local_3 < _badWords.length)
            {
        ...
        public function disposeWindow():void
        {
            ...
            {
                _badWordList.removeListItems();
                _badWordList.dispose();
                _badWordList = null;
            };

            if (_badWordField)
            {
                _badWordField.dispose();
                _badWordField = null;
            };
