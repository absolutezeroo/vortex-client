package com.sulake.habbo.navigator.roomsettings
{
    ...
    public class RoomCreateViewCtrl 
    {
        ...
        {
            _navigator = _arg_1;
            _arrowTimer = new Timer(100);
            _arrowTimer.addEventListener("timer", updateArrowPos);
        ...
        public function dispose():void
        {
            if (_arrowTimer)
            {
                _arrowTimer.removeEventListener("timer", updateArrowPos);
                _arrowTimer.reset();
                _arrowTimer = null;
            };
        }
        ...
        private function updateArrowPos(_arg_1:Event):void
        {
            ...
            _local_3.y = (_local_3.y + ((_arrowMovingDown) ? _local_4 : -(_local_4)));

            if (_local_3.y < _local_2)
            {
                _arrowMovingDown = true;
        ...
        private function prepare():void
        {
            ...
            _list = IItemListWindow(_content.findChildByName("layout_item_list"));
            ...
            _roomNameInput = new TextFieldManager(_navigator, ITextFieldWindow(_content.findChildByName("room_name_input")), 25, null, _navigator.getText("navigator.createroom.roomnameinfo"));
            _roomDescInput = new TextFieldManager(_navigator, ITextFieldWindow(_content.findChildByName("room_desc_input")), 128, null, _navigator.getText("navigator.createroom.roomdescinfo"));
        ...
        {
            _categoryMenu = (_content.findChildByName("categories_list") as IDropMenuWindow);
            ...
            for each (var _local_2:FlatCategory in _navigator.data.visibleCategories)
            {
                ...
                {
                    _selectableAndVisibleCategories.push(_local_2);
                    _local_1.push(_local_2.visibleName);
                };
            };

            _categoryMenu.populate(_local_1);
            _categoryMenu.selection = 0;
        }

        private function prepareTradeModeSelection():void
        {
            _tradeSettings = (_content.findChildByName("trade_settings_list") as IDropMenuWindow);
            ...
            _tradeSettings.populate(_local_1);
            _tradeSettings.selection = 0;
        }

        public function refresh():void
        {
            _roomNameInput.goBackToInitialState();
            _roomNameInput.input.textBackgroundColor = 0xFFFFFFFF;
            _roomDescInput.goBackToInitialState();
            _roomDescInput.input.textBackgroundColor = 0xFFFFFFFF;
            _tradeSettings.selection = 0;
            _categoryMenu.selection = 0;
        ...
        private function refreshRoomThumbnails():void
        {
            ...
            while (_list.numListItems > 0)
            {
                _local_9 = _list.removeListItemAt(0);
                _local_9.destroy();
            };
        ...
        {
            return (_roomNameInput.checkMandatory(_navigator.getText("navigator.createroom.nameerr")));
        }
        ...
        {
            var _local_8:String = _roomNameInput.getText();
            var _local_3:String = _roomDescInput.getText();
            ...
            for each (var _local_7:FlatCategory in _selectableAndVisibleCategories)
            {
                if (_categoryMenu.selection == _local_4)
                {
            ...
            var _local_6:int = _tradeSettings.selection;
