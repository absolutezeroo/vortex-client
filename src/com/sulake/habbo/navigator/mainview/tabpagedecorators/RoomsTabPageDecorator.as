package com.sulake.habbo.navigator.mainview.tabpagedecorators
{
    ...
    public class RoomsTabPageDecorator implements ITabPageDecorator 
    {
        ...
        public function refreshCustomContent(_arg_1:IWindowContainer):void
        {
            ...
            if (((_filter == null) || (_filter.disposed)))
            {
                _filter = IDropMenuWindow(_local_3.findChildByName("roomCtgFilter"));
                prepareRoomCategories();
                _filter.addEventListener("WE_SELECTED", onFilterSelected);
            };
        ...
        public function prepareRoomCategories():void
        {
            ...
            _personalizedRoomsEnabled = _navigator.context.configuration.getBoolean("navigator.2014.personalized.navigator");

            if (_personalizedRoomsEnabled)
            {
            ...
            _filter.populate(_local_1);
            _filter.selection = defaultSelection;
        }

        private function get defaultSelection():int
        {
            return ((_personalizedRoomsEnabled) ? 2 : 0);
        }
        ...
        public function tabSelected():void
        {
            if (((!(_filter == null)) && (!(_filter.disposed))))
            {
                _filter.removeEventListener("WE_SELECTED", onFilterSelected);
                _filter.selection = defaultSelection;
                _filter.addEventListener("WE_SELECTED", onFilterSelected);
            };
        }
        ...
        private function startSearch():void
        {
            ...
            var _local_1:int;
            var _local_2:int = (((_filter) && (!(_filter.disposed))) ? _filter.selection : defaultSelection);
            ...
            else
            {
                ...
                {
                    if (((_local_2 == 2) && (_personalizedRoomsEnabled)))
                    {
            ...
            if (((_filter) && (!(_filter.disposed))))
            {
                _navigator.trackNavigationDataPoint(_filter.enumerateSelection()[_filter.selection], "category.view");
            };
        }

        public function get filterCategory():String
        {
            return (((_filter) && (!(_filter.disposed))) ? _filter.enumerateSelection()[_filter.selection] : null);
        }
