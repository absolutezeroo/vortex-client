        ...
        private function onError(event:IMessageEvent):void
        {
            ...
            switch (error.getParser().errorCode)
            {
                case -100002:
                    ...
                    return;
                case 4009:
                    _navigator.windowManager.alert("${generic.alert.title}", "${navigator.alert.need.to.be.vip}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                    {
                        event.dispose();
                    });
                    return;
                case 4010:
                    _navigator.windowManager.alert("${generic.alert.title}", "${navigator.alert.invalid_room_name}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                    {
                        event.dispose();
                    });
                    return;
                case 4011:
                    _navigator.windowManager.alert("${generic.alert.title}", "${navigator.alert.cannot_perm_ban}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                    {
                        event.dispose();
                    });
                    return;
                case 4013:
                    _navigator.windowManager.alert("${generic.alert.title}", "${navigator.alert.room_in_maintenance}", 0, function (event:IAlertDialog, _arg_2:WindowEvent):void
                    {
                        event.dispose();
                    });
                    return;
