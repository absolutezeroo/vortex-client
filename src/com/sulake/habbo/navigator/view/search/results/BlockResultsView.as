package com.sulake.habbo.navigator.view.search.results
{
    ...
    public class BlockResultsView 
    {
        ...
        private var _searchCodeShowMore:Dictionary = new Dictionary();
        private var _currentWindowsInItemList:Dictionary = new Dictionary();
        ...
        {
            _categoryElementFactory = _arg_1;
        }
        ...
        public function displayCurrentResults():void
        {
            ...
            _currentWindowsInItemList = new Dictionary();
            _searchCodeShowMore = new Dictionary();
            _searchCodeViewMode = new Dictionary();
            _itemList.destroyListItems();

            if (_navigator.currentResults.resultSet.blocks.length == 0)
            {
                _itemList.addListItem(_categoryElementFactory.getNoResultsELement());
            }
            ...
            else
            {
                ...
                while (_local_2 < _local_4.blocks.length)
                {
                    ...
                    _currentWindowsInItemList[_local_2] = _local_1;
                    _searchCodeShowMore[_local_2] = _local_3.searchCode;
                    _searchCodeViewMode[_local_2] = _local_3.viewMode;
                    _local_2++;
                };
            };
        ...
        {
            _navigator.performSearch(_searchCodeShowMore[_arg_1.window.id], _navigator.currentResults.filteringData);
            _navigator.trackEventLog("browse.expandsearch", "Results", HabboNewNavigator.getEventLogExtraStringFromSearch(_searchCodeShowMore[_arg_1.window.id], _navigator.currentResults.filteringData));
        }
        ...
        {
            _navigator.sendAddCollapsedCategory(_searchCodeShowMore[_arg_1.window.id]);
            _navigator.collapsedCategories.push(_searchCodeShowMore[_arg_1.window.id]);
            replaceBlock(_arg_1.window.id, false);
            _navigator.trackEventLog("browse.collapsecategory", "Results", HabboNewNavigator.getEventLogExtraStringFromSearch(_searchCodeShowMore[_arg_1.window.id], _navigator.currentResults.filteringData));
        }

        public function onCategoryExpandClicked(_arg_1:WindowEvent):void
        {
            _navigator.sendRemoveCollapsedCategory(_searchCodeShowMore[_arg_1.window.id]);
            _navigator.collapsedCategories.splice(_navigator.collapsedCategories.indexOf(_searchCodeShowMore[_arg_1.window.id]), 1);
            replaceBlock(_arg_1.window.id, true);
            _navigator.trackEventLog("browse.uncollapsecategory", "Results", HabboNewNavigator.getEventLogExtraStringFromSearch(_searchCodeShowMore[_arg_1.window.id], _navigator.currentResults.filteringData));
        }

        public function onCategoryAddQuickLinkClicked(_arg_1:WindowEvent):void
        {
            _navigator.addSavedSearch(_searchCodeShowMore[_arg_1.window.id], _navigator.currentResults.filteringData);
        }

        public function onCategoryToggleModeClicked(_arg_1:WindowEvent):void
        {
            var _local_2:int = getToggledViewMode(_searchCodeViewMode[_arg_1.window.id]);
            _navigator.toggleSearchCodeViewMode(_searchCodeShowMore[_arg_1.window.id], _local_2);
            _navigator.currentResults.resultSet.blocks[_arg_1.window.id].viewMode = _local_2;
            replaceBlock(_arg_1.window.id, true);
            _searchCodeViewMode[_arg_1.window.id] = _local_2;
        }
        ...
        private function renderCurrentResultsBlock(_arg_1:int, _arg_2:Boolean):IWindow
        {
            ...
            if (_arg_2)
            {
                ...
                return (_categoryElementFactory.getOpenCategoryElement(_local_3.guestRooms, _local_4, _arg_1, _local_3.actionAllowed, _local_5));
            };

            return (_categoryElementFactory.getCollapsedCategoryElement(_local_4, _arg_1, _local_3.actionAllowed));
        }

        private function replaceBlock(_arg_1:int, _arg_2:Boolean):void
        {
            var _local_4:int = _itemList.getListItemIndex(_currentWindowsInItemList[_arg_1]);
            ...
            _currentWindowsInItemList[_arg_1] = _local_3;
        }
