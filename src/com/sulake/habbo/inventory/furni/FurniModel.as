package com.sulake.habbo.inventory.furni
{
    ...
    public class FurniModel implements IInventoryModel
    {
        ...
        public function FurniModel(_arg_1:HabboInventory, _arg_2:MarketplaceModel, _arg_3:IHabboWindowManager, _arg_4:IHabboCommunicationManager, _arg_5:IAssetLibrary, _arg_6:IRoomEngine, _arg_7:IHabboCatalog, _arg_8:IHabboSoundManager, _arg_9:IHabboLocalizationManager)
        {
            ...
            _bottomAlignedFurniture = _controller.getProperty("catalog.preview.alignment.bottom").split(",");
            _imageUpdateTimer = new Timer(100);
            _imageUpdateTimer.addEventListener("timer", onImageUpdateTimerEvent);
            _categorySelection = new Dictionary();
            _categorySelection["furni"] = null;
            _categorySelection["rentables"] = null;
            clearFurniList();
            _view = new FurniView(this, _arg_2, _windowManager, _assets, _roomEngine);
        }
        ...
        {
            _categorySelection[_category] = _arg_1;
        }
        ...
        public function createItemWindow(_arg_1:String):IWindowContainer
        {
            ...
            if (_thumbWindowTemplates[_arg_1] == null)
            {
                _local_4 = _assets.getAssetByName(_arg_1);
                _local_2 = XmlAsset(_local_4);
                _local_3 = XML(_local_2.content);
                _thumbWindowTemplates[_arg_1] = (_windowManager.buildFromXML(_local_3) as IWindowContainer);
            };

            return (_thumbWindowTemplates[_arg_1].clone());
        }
        ...
        public function createGroupItem(_arg_1:int, _arg_2:int, _arg_3:IStuffData, _arg_4:Number=NaN, _arg_5:Boolean=false):GroupItem
        {
            ...
            if (_bottomAlignedFurniture.indexOf(_local_6) > -1)
            {
                _local_9 = "bottom";
            };
        ...
        public function categorySwitch(_arg_1:String):void
        {
            if (_controller.isVisible)
            {
                ...
                switch (_arg_1)
                {
                    ...
                    case "rentables":
                        _category = _arg_1;
                        _showingRentedFurni = (_category == "rentables");
                        _view.resetFilters(_arg_1);
        ...
        public function removeFurni(_arg_1:int):GroupItem
        {
            ...
            while (_local_4 < _furniData.length)
            {
                ...
                if (_local_2)
                {
                    if (_itemIdInFurniPlacing == _local_2.ref)
                    {
                        cancelFurniInMover();
                    ...
                    if (_local_3.getTotalCount() <= 0)
                    {
                        ...
                        if (((_view) && (_view.grid)))
                        {
                            _view.grid.itemWasUpdated(_local_3);
                        };
        ...
        {
            var _local_2:IWindowContainer = _view.getFirstThumb();
        ...
        {
            if (_itemIdInFurniPlacing > -1)
            {
                _roomEngine.cancelRoomObjectInsert();
                _isObjectMoverRequested = false;
                _itemIdInFurniPlacing = -1;
            };
        }
        ...
        private function initListImages():void
        {
            ...
            if (!_imageUpdateTimer.running)
            {
                _imageUpdateTimer.start();
            };

            if (((_view) && (_view.currentPageItems)))
            {
                for each (_local_1 in _view.currentPageItems)
                {
        ...
        private function addOrUpdateGroupableItem(_arg_1:FurnitureItem, _arg_2:Boolean):GroupItem
        {
            ...
            if (_local_4)
            {
                ...
                if (((_view) && (_view.grid)))
                {
                    _view.grid.itemWasUpdated(_local_4);
                };
        ...
        private function requestSelectedFurniToMover(_arg_1:FurnitureItem):void
        {
            ...
            if (_local_2)
            {
                _itemIdInFurniPlacing = _arg_1.ref;
                requestClose();
                _isObjectMoverRequested = true;
            };
        ...
        private function updateCategorySelection():void
        {
            ...
            var _local_1:GroupItem = _categorySelection[_category];
        ...
        {
            if (((_isObjectMoverRequested) && (_arg_1.type == "REOE_PLACED")))
            {
                _isObjectMoverRequested = false;
        ...
        public function roomEntered():void
        {
            ...
            if (_roomEngine != null)
            {
                _local_1 = ((_roomItemToSelect.isWallItem) ? 20 : 10);
                _roomEngine.selectRoomObject(_roomItemToSelect.flatId, Math.abs(_roomItemToSelect.id), _local_1);
            };
