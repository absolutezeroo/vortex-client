package com.sulake.habbo.friendbar.talent
{
    ...
    public class TalentLevelUpController implements IDisposable 
    {
        ...
        public function dispose():void
        {
            if (!_disposed)
            {
                if (_rewardPerkTemplate != null)
                {
                    _rewardPerkTemplate.dispose();
                    _rewardPerkTemplate = null;
                };

                if (_rewardProductTemplate != null)
                {
                    _rewardProductTemplate.dispose();
                    _rewardProductTemplate = null;
                };

                if (_rewardVipTemplate != null)
                {
                    _rewardVipTemplate.dispose();
                    _rewardVipTemplate = null;
                };
        ...
        public function showWindow(_arg_1:String, _arg_2:int, _arg_3:Vector.<TalentTrackRewardPerk>, _arg_4:Vector.<TalentTrackRewardProduct>):void
        {
            ...
            _rewardProductTemplate = _local_8.removeListItem(_local_8.getListItemByName("reward_product_template"));
            _rewardVipTemplate = _local_8.removeListItem(_local_8.getListItemByName("reward_vip_template"));
            _rewardPerkTemplate = _local_8.removeListItem(_local_8.getListItemByName("reward_perk_template"));
        ...
        {
            var _local_2:IWindowContainer = (_rewardPerkTemplate.clone() as IWindowContainer);
        ...
        private function createRewardProduct(_arg_1:TalentTrackRewardProduct):IWindow
        {
            ...
            if (_arg_1.vipDays == 0)
            {
                _local_2 = _rewardProductTemplate.clone();
            ...
            {
                _local_2 = _rewardVipTemplate.clone();
        ...
        private function onWindowEvent(_arg_1:WindowEvent, _arg_2:IWindow):void
        {
            ...
            switch (_arg_2.name)
            {
                ...
                case "talent_button":
                    closeWindow();
                    _habboTalent.tracking.trackTalentTrackOpen(_track, "levelup");
                    _habboTalent.send(new GetTalentTrackMessageComposer(_track));
                    return;
            };
        }
