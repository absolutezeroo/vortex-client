package com.sulake.habbo.friendbar.landingview.widget
{
    ...
    public class GenericWidget implements ILandingViewWidget, ISlotAwareWidget, ISettingsAwareWidget, IConfigurationCodeAwareWidget, IDisableAwareWidget 
    {
        ...
        public function dispose():void
        {
            ...
            for each (var _local_1:IElementHandler in _elements)
            {
        ...
        public function initialize():void
        {
            ...
            configureLayout(_landingView, _slot, _configurationCode, _container);
        }

        public function getElementByName(_arg_1:String):IElementHandler
        {
            return (_elements.getValue(_arg_1) as IElementHandler);
        }
        ...
        private function configureContentColumn():void
        {
            ...
            var _local_2:String = getConf(_landingView, _slot, _configurationCode, "conf");
            ...
            for each (var _local_3:String in _local_7)
            {
                ...
                if (_local_1 != null)
                {
                    ...
                    _elements.add(_local_8, _local_1);
                };

                if (((_local_1 is IFloatingElement) && (IFloatingElement(_local_1).isFloating(isWideSlot(_slot)))))
                {
                    if ((_local_1 is TitleElementHandler))
                    {
                        _local_5.width = ((isWideSlot(_slot)) ? _landingView.dynamicLayoutLeftPaneWidth : _landingView.dynamicLayoutRightPaneWidth);
                    };
