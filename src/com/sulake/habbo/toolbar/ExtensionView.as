package com.sulake.habbo.toolbar
{
    ...
    public class ExtensionView implements IExtensionView 
    {
        ...
        public function ExtensionView(_arg_1:IHabboWindowManager, _arg_2:IAssetLibrary, _arg_3:HabboToolbar)
        {
            ...
            if (_local_5)
            {
                _itemWindow = (_arg_1.buildFromXML((_local_5.content as XML), 1) as IItemListWindow);
            };

            var _local_4:IWindowContainer = _itemWindow.desktop;

            if (_itemWindow)
            {
                _itemWindow.x = (((_local_4.width - _itemWindow.width) - 3) - extraMargin);
                _itemWindow.y = 3;
        ...
        {
            return ((_itemWindow) && (_itemWindow.visible));
        }
        ...
        public function get screenHeight():uint
        {
            ...
            return (_itemWindow.height + _itemWindow.y);
        }
        ...
        public function attachExtension(_arg_1:String, _arg_2:IWindow, _arg_3:int=-1, _arg_4:Array=null):void
        {
            if (!_disposed)
            {
                ...
                if (_arg_3 == -1)
                {
                    _itemWindowElements.push(_arg_2);
                }

                else
                {
                    _itemWindowElements.splice(_arg_3, 0, _arg_2);
                };
        ...
        {
            var _local_2:String;
            _itemWindow.removeListItems();

            for each (var _local_1:IWindow in _itemWindowElements)
            {
                ...
                switch (_local_2)
                {
                    ...
                    case "return_gift":
                    case "targeted_offer":
                        _itemWindow.addListItem(_local_1);
                        break;
                    case "purse":
                        _itemWindow.addListItem(_local_1);
                        _itemWindow.y = (3 + -8);
            ...
            _itemWindow.arrangeListItems();
            _itemWindow.invalidate();
        }
        ...
        private function resolveIndex(_arg_1:Array):int
        {
            ...
            while (_local_2 < _itemWindowElements.length)
            {
                if (_arg_1.indexOf(_itemWindowElements[_local_2].name) > -1)
                {
                    return (_local_2);
                };
        ...
        public function detachExtension(_arg_1:String):void
        {
            ...
            if (!_disposed)
            {
                ...
                if (_local_2 != null)
                {
                    _itemWindowElements.splice(_itemWindowElements.indexOf(_local_2), 1);
        ...
        public function set visible(_arg_1:Boolean):void
        {
            if (_itemWindow)
            {
                _itemWindow.visible = _arg_1;
            };
        }

        public function removeDimmers():void
        {
            for each (var _local_1:IWindowContainer in _itemWindowElements)
            {
        ...
        public function set extraMargin(_arg_1:int):void
        {
            ...
            if (_itemWindow)
            {
                _itemWindow.x = (((_itemWindow.desktop.width - _itemWindow.width) - 3) - extraMargin);
            };
        }
