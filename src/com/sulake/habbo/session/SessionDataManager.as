...
//                    context.displayObjectContainer.stage.nativeWindow.title = ((("Habbo " + _local_4.toUpperCase()) + " | ") + _userName);
        ...
        {
            var roomId:int = userId;
            var message:String = userName;
            ...
            if ((((session.isRoomOwner) || (isAnyRoomController)) || (session.roomControllerLevel >= 1)))
            {
                _windowManager.confirm("${generic.alert.title}", "${room.confirm.eject_all}", 0, function (userId:_SafeStr_126, userName:WindowEvent):void
                {
                    userId.dispose();

                    if (userName.type == "WE_OK")
                    {
        ...
        public function ejectPets(roomId:int):void
        {
            ...
            var _local_2:IRoomSession = _roomSessionManager.getSession(roomId);
