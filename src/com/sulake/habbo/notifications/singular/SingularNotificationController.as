package com.sulake.habbo.notifications.singular
{
    ...
    public class SingularNotificationController implements IUpdateReceiver 
    {
        ...
        public function SingularNotificationController(_arg_1:HabboNotifications)
        {
            ...
            _achievementNotifications = new Dictionary();
            ...
            _notificationViewManager = new HabboNotificationViewManager(_notifications, _notifications.assetLibrary, _notifications.windowManager, _notifications.toolBar, _styleConfiguration["styles"], _styleConfiguration["view"]);
        ...
        {
            if (_moderationDelayTimer)
            {
                _moderationDelayTimer.removeEventListener("timerComplete", onShowModerationDisclaimer);
                _moderationDelayTimer = null;
            };

            if (_notificationViewManager != null)
            {
                _notificationViewManager.dispose();
                _notificationViewManager = null;
            };
        ...
        public function update(_arg_1:uint):void
        {
            ...
            if (((_notificationViewManager.isSpaceAvailable()) && (_notificationQueue.length > 0)))
            {
                _local_2 = getNextItemFromQueue();

                if (!_notificationViewManager.showItem(_local_2))
                {
                    _local_2.dispose();
                };
            };
        ...
        public function showModerationDisclaimer():void
        {
            ...
            {
                if (_moderationDelayTimer == null)
                {
                    _moderationDelayTimer = new Timer((RoomEnterEffect.totalRunningTime + 5000), 1);
                    _moderationDelayTimer.addEventListener("timerComplete", onShowModerationDisclaimer);
                    _moderationDelayTimer.start();
                };
            }

            else
            {
                if (!_moderationDisclaimerShown)
                {
                    _local_1 = _notifications.localization.getLocalization("mod.chatdisclaimer", "NA");
                    this.addItem(_local_1, "info", null);
                    _moderationDisclaimerShown = true;
        ...
        {
            if (((!(_arg_1 == null)) && (!(this._notificationViewManager == null))))
            {
                _notificationViewManager.replaceIcon(_arg_1);
            };
        }
