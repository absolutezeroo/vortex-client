package com.sulake.habbo.tracking
{
    ...
    public class HabboTracking extends Component implements IHabboTracking, IUpdateReceiver
    {
        ...
        public function HabboTracking(_arg_1:IContext, _arg_2:uint=0, _arg_3:IAssetLibrary=null)
        {
            ...
            _errorContextFlags = new Array(11);
            _local_5 = 0;

            while (_local_5 < 11)
            {
                _errorContextFlags[_local_5] = 0;
                _local_5++;
            };
        ...
        {
            _latencyTracker = new LatencyTracker(this);
            _performanceTracker = new PerformanceTracker(this);
            _framerateTracker = new FramerateTracker(this);
            _lagWarningLogger = new LagWarningLogger(this);
            _toolbarClickTracker = new ToolbarClickTracker(this);
        ...
        override public function dispose():void
        {
            ...
            _framerateTracker = null;
            _toolbarClickTracker = null;
            ...
            if (_fpsTrackingTimer)
            {
                _fpsTrackingTimer.stop();
                _fpsTrackingTimer.removeEventListener("timer", onRoomActionTimerEvent);
                _fpsTrackingTimer = null;
            };
        ...
        {
            _errorContextFlags[_arg_2] = _arg_1;
        }
        ...
        private function storeErrorDetails(_arg_1:ErrorEvent):void
        {
            ...
            for each (var _local_2:uint in _errorContextFlags)
            {
            ...
            {
                ErrorReportStorage.setParameter("flash_version", _performanceTracker.flashVersion);
                ErrorReportStorage.setParameter("avg_update", String(_performanceTracker.averageUpdateInterval));
            };
        ...
        private function onPingResponse(_arg_1:LatencyPingResponseMessageEvent):void
        {
            if (_latencyTracker != null)
            {
                _latencyTracker.onPingResponse(_arg_1);
            };
        }
        ...
        private function onRoomAction(_arg_1:RoomEngineEvent):void
        {
            if (_arg_1.type == "REE_INITIALIZED")
            {
                if (!_fpsTrackingTimer)
                {
                    _fpsTrackingRoomId = _arg_1.roomId;
                    _fpsTrackingTimer = new Timer(60000, 1);
                    _fpsTrackingTimer.addEventListener("timer", onRoomActionTimerEvent);
                    _fpsTrackingTimer.start();
                };
            }
        ...
        private function onRoomActionTimerEvent(_arg_1:TimerEvent):void
        {
            ...
            if (((((!(disposed)) && (!(_crashed))) && (!(_communication == null))) && (_framerateTracker)))
            {
                _local_2 = null;
                ...
                trackEventLog("ClientPerformance", String(_framerateTracker.frameRate), "fps", _local_2, _fpsTrackingRoomId);
                _fpsTrackingCount++;
            };
        }
        ...
        {
            if (_toolbarClickTracker)
            {
                _toolbarClickTracker.track(_arg_1.iconName);
            };
        }
        ...
        private function onZoomToggle(_arg_1:HabboToolbarEvent):void
        {
            if (_toolbarClickTracker)
            {
                _toolbarClickTracker.track(_arg_1.type);
            };
        }
        ...
        {
            _lagWarningLogger.chatLagDetected(_arg_1);
        }
        ...
        public function update(_arg_1:uint):void
        {
            ...
            {
                _invalidTimeCount++;
                ErrorReportStorage.addDebugData("Invalid time counter", ("Invalid times: " + _invalidTimeCount));
            };

            if (((_currentTime > -1) && ((_local_2 - _currentTime) > 15000)))
            {
                _timeLeapCount++;
                ErrorReportStorage.addDebugData("Time leap counter", ("Time leaps: " + _timeLeapCount));
            };
            ...
            if (_performanceTracker != null)
            {
                _performanceTracker.update(_arg_1, _currentTime);
            };

            if (_latencyTracker != null)
            {
                _latencyTracker.update(_arg_1, _currentTime);
            };

            if (_framerateTracker != null)
            {
                _framerateTracker.trackUpdate(_arg_1, _currentTime);
            };

            if (_lagWarningLogger != null)
            {
                _lagWarningLogger.update(_currentTime);
            };
        }
